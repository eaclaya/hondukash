...
import { DiscountCalculationContext, PricingRule, CartItem } from '@/lib/types/discounts';
...
export function useDiscountCalculation({ selectedClient, items, storeId }: UseDiscountCalculationProps) {
  ...
  const itemsCalculationKey = useMemo(() => {
    return items.map((item) => `${item.productId}-${item.quantity}-${item.unitPrice}-${item.productName}`).join('|');
  }, [items]);
          ...
          ? [
              {
                ...
                description: finalRule ? `${finalRule.name} - ${finalRule.ruleType}` : `Client discount applied`
              }
            ]
          : [];
  ...
  const calculateDiscountsWithProductDetails = useCallback(
    async (productsWithInventory: ProductWithInventory[]): Promise<DiscountSummary> => {
      if (!selectedClient || items.length === 0) {
        const originalSubtotal = items.reduce((sum, item) => sum + item.quantity * item.unitPrice, 0);
