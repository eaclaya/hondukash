...
export class DiscountEngine {
   ...
   */
  static async calculateDiscounts(context: DiscountCalculationContext, rules: PricingRule[]): Promise<DiscountCalculationResult> {
    // Filter and sort active rules by priority
    const activeRules = rules.filter((rule) => this.isRuleActive(rule, context.currentDate)).sort((a, b) => b.priority - a.priority); // Higher priority first
   ...
   */
  private static evaluateRuleConditions(rule: PricingRule, context: DiscountCalculationContext, currentSubtotal: number): boolean {
   ...
   */
  private static evaluateConditionGroup(conditions: RuleCondition[], context: DiscountCalculationContext, currentSubtotal: number): boolean {
    let result = true;
   ...
   */
  private static evaluateCondition(condition: RuleCondition, context: DiscountCalculationContext, currentSubtotal: number): boolean {
    ...
    switch (conditionType) {
      ...
      case 'client_has_any_tags':
        return context.clientTags.some((tag) => (condition.valueArray ? condition.valueArray.includes(tag) : false));
      case 'product_has_tag':
        return context.cartItems.some((item) => this.evaluateArrayCondition(item.productTags, condition));
      case 'product_has_any_tags':
        return context.cartItems.some((item) => (condition.valueArray ? item.productTags.some((tag) => condition.valueArray!.includes(tag)) : false));
      case 'product_quantity':
        // Sum quantity for specific product
        actualValue = context.cartItems.filter((item) => item.productId === condition.valueText).reduce((sum, item) => sum + item.quantity, 0);
        break;
      case 'product_category':
        return context.cartItems.some((item) => this.evaluateArrayCondition([item.categoryId || ''], condition));
      case 'product_sku':
        return context.cartItems.some((item) => this.evaluateStringCondition(item.sku, condition));
      case 'day_of_week':
   ...
   */
  private static evaluateOperator(actualValue: number, operator: ConditionOperator, condition: RuleCondition): boolean {
   ...
   */
  private static applyRule(rule: PricingRule, cartItems: CartItem[], currentSubtotal: number): AppliedDiscount | null {
    // Get target items
    ...
    switch (rule.ruleType) {
      ...
      case 'buy_x_get_y':
        ...
        totalDiscountAmount = appliedToItems.reduce((sum, item) => sum + item.discountAmount * item.quantity, 0);
        break;
  ...
  private static getTargetItems(rule: PricingRule, cartItems: CartItem[]): CartItem[] {
    ...
    for (const target of rule.targets) {
      switch (target.targetType) {
        ...
        case 'specific_products':
          if (target.targetIds) {
            targetItems.push(...cartItems.filter((item) => target.targetIds!.includes(item.productId)));
          }
          break;
        case 'products_with_tag':
          if (target.targetTags) {
            targetItems.push(...cartItems.filter((item) => target.targetTags!.some((tag) => item.productTags.includes(tag))));
          }
          break;
        case 'products_with_any_tags':
          if (target.targetTags) {
            targetItems.push(...cartItems.filter((item) => item.productTags.some((productTag) => target.targetTags!.includes(productTag))));
          }
          break;
        case 'product_category':
          if (target.targetIds) {
            targetItems.push(...cartItems.filter((item) => item.categoryId && target.targetIds!.includes(item.categoryId)));
          }
          break;
        case 'cheapest_item':
          const cheapest = cartItems.reduce((min, item) => (item.unitPrice < min.unitPrice ? item : min));
          targetItems.push(cheapest);
          break;
        case 'most_expensive_item':
          const mostExpensive = cartItems.reduce((max, item) => (item.unitPrice > max.unitPrice ? item : max));
    ...
    // Remove duplicates
    return targetItems.filter((item, index, self) => index === self.findIndex((i) => i.productId === item.productId));
  }
  ...
  private static findApplicableTier(tiers: QuantityPriceTier[], quantity: number): QuantityPriceTier | null {
    return (
      tiers
        .filter((tier) => quantity >= tier.minQuantity && (tier.maxQuantity === null || quantity <= tier.maxQuantity))
